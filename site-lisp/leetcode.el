...
(defun leetcode-menu--parser1 (&rest resp)
  (let ((stat_status_pairs (assoc 'stat_status_pairs resp)))
    (message stat_status_pairs)))
...
(defun leetcode-logged ()
  (leetcode-cookie-val "LEETCODE_SESSION"))
...
(defun leetcode-fetch-menu (&rest _)
  ...
  (with-current-buffer (get-buffer-create "*Leetcode*")
    (leetcode-menu-mode)
    (tabulated-list-init-header)    
    (request (concat lc-site-prefix "/api/problems/algorithms/")
	     :headers (lc-request-header)
             :parser 'json-read
	     :complete 'leetcode-menu--parser1
	     :sync t)
