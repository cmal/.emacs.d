...
;; Privoxy 
(defcustom proxy-mode-env-http-proxy
  "http://localhost:8118"
...
(defcustom proxy-mode-emacs-socks-proxy
...
(defun proxy-mode-emacs-http-enable ()
  ...
  (message 
   (format "Proxy-mode %s url proxy enabled." 
	   (car proxy-mode-emacs-http-proxy))))  

(defun proxy-mode--emacs-http-disable ()   "Disable URL proxy."   
    (setq-local url-proxy-services nil)   
    (setq-local proxy-mode-proxy-type nil))  

;;; ------------------------------ Socks Proxy --------------------------------------------------  
(defun proxy-mode-emacs-socks-enable ()
  "Enable Socks proxy. NOTE: it only works for http:// connections. Not work for https:// connections."   
    (require 'url-gw)   
    (require 'socks)   
...
(defun proxy-mode--emacs-socks-disable ()   "Disable Socks proxy."   
...
(defun proxy-mode-enable ()
  "Enable proxy-mode."   
    (cl-case proxy-mode-proxy-type     
      ('env-http 
       (proxy-mode-env-http-enable))     
      ('emacs-socks 
       (proxy-mode-emacs-socks-enable))     
      ('emacs-http 
       (proxy-mode-emacs-http-enable))))  

(defun proxy-mode-disable ()
  "Disable proxy-mode."   
    (pcase proxy-mode-proxy-type     
      ('env-http 
       (proxy-mode--env-http-disable))     
      ('emacs-socks 
       (proxy-mode--emacs-socks-disable))     
      ('emacs-http 
       (proxy-mode--emacs-http-disable))))  

(defvar proxy-mode-map nil)  

;;;###autoload 
(define-minor-mode proxy-mode
  "A minor mode to toggle `proxy-mode' buffer locally. This minor mode supports buffer-local proxy: Emacs http, and Emacs socks. Not support buffer-locally shell environment variable HTTP_PROXY. If you want use proxy-mode globally, use command ‘global-proxy-mode’."
  :require 'proxy-mode
  :init-value nil
  :lighter (:eval 
  ...
  "Enable proxy in Emacs globally."   
    ...
    (cl-case proxy-mode-proxy-type     
      ...
      ('emacs-http      
       ...
       (message 
	(format "Proxy-mode %s url proxy enabled globally." 
		(car proxy-mode-emacs-http-proxy))))))  
